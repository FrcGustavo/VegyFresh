...
export default function UsersList() {
  ...
  return <UsersTable list={list} onDelete={(id) => deleteMutation.mutate(id)} />;
}

function UsersTable({ list, onDelete }: { list: any[]; onDelete: (id: string) => void }) {
  ...
  return (
    <div>
      <Typography variant="h4" gutterBottom>Lista de Users</Typography>
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 3 }}>
        <Button component={Link} to="/users/create" variant="contained" color="primary">
          Crear Nuevo
        </Button>
        <TextField
          size="small"
          ...
          <TableBody>
            {paginated.length === 0 ? (
              ...
              <TableRow key={item.id}>
                ...
                <TableCell>
                  <Tooltip title="Ver"><IconButton component={Link} to={`/users/${item.id}`} color="primary"><ViewIcon /></IconButton></Tooltip>
                  <Tooltip title="Editar"><IconButton component={Link} to={`/users/${item.id}/edit`} color="secondary"><EditIcon /></IconButton></Tooltip>
